...
# Inventory Class
    class Inventory(object):
        def __init__(self):
            ...
            self.width = inv_settings.width
            self.height = inv_settings.height
        ...
        def add(self, item, count=1):
            # Check if the item is an instance of Armor class
            if isinstance(item, Armor):
                # Check if the item's rarity is already in the inventory
                if item.rarity not in self.inventory:
                    # Add the item to the inventory with the specified count
                    self.inventory[item.rarity] = {item: count}
                else:
                    # Check if the item is already in the inventory
                    if item in self.inventory[item.rarity]:
                ...
                # Handle non-Armor items (similar to your existing logic)
            ...
            # Commented out means there's no limit to stacks
        ...
        def use(self, item = None):
            ...
            if self.selected_item:
                ...
                # Check if the health items are consumed
                if self.selected_item.consumed_on_use and player_hp >= player_hp_max:
                    return
                else:
                    # Remove the Item from the Inventory.
                    self.remove()
